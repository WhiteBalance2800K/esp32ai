...
- **桌宠外观**：依次切换经典宠物 / 照片定制咖色边牧 / 自定义；“自定义…”会打开
  [petdex.dev](https://petdex.dev) 画廊（3300+ 开源桌宠），搜索 → 选动画 → 预览 →
  一键上传到设备；同一菜单还可选择 70% / 85% / 100% 显示大小
...
1. **Mac 菜单栏 →「桌宠外观 → 自定义…」**：从 [petdex.dev](https://petdex.dev) 的公开
...
桥接端按菜单选择的 10/30/60/120 秒间隔更新当前行情数据，并在相同间隔内在最多 15 个收藏标的之间轮换（默认 10 秒），支持 A 股、港股、
...
- `GET /btc/version`：设备每秒检查一次的轻量版本信息，包含 codec、压缩长度和桥接进程
  session；画面不变时不会重复下载，桥接重启或系统时钟回拨也不会卡住旧版本
- `GET /btc/frame.rle`：ESP32-C3 使用的 PackBits RGB565 帧，含版本、尺寸和 CRC32
- `GET /btc/frame.raw`：兼容旧固件的 240x240 RGB565 大端完整帧

压缩帧通常只有原始 115,200 字节的约 10%–15%。C3 先把它完整接收进 32 KiB 暂存区，校验
CRC、尺寸和像素数后才快速写屏；断包或校验失败时保留旧画面，不再出现按网络速度逐块爬屏。
设备连续两次无法读取压缩帧时会限频尝试 raw 兼容路径，不会永久冻结。桥接端在当前标的驻留
期间预取并渲染后继标的，只切换已经完整准备好的帧；单个数据源失败会退避 30 秒并继续预取
后续收藏，因此不会形成队头阻塞。

`/status` 所需的本机会话日志可能达到数百 MB。日志扫描现在放在独立后台队列，HTTP 路径采用
stale-while-revalidate：轮询立即拿到上一份完整状态，扫描完成后原子替换缓存。这样慢日志扫描
不会再阻塞 `/btc/version` 和行情帧传输；hook、额度和音乐状态仍在每次响应时即时叠加。
