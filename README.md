...
`0.4.6-c3.15` 后连接 ESP32-C3。当前 Web 固件已在上述成品板实机点亮；普通升级（不勾选
...
| | |
|---|---|
| <img src="docs/images/feature1.jpg" width="360" alt="AI 工作状态"> | **AI 工作状态、额度与当日成本**<br>桌宠动起来 = AI 正在干活；内置经典宠物和照片定制的咖色边牧，忙时向左奔跑、空闲时坐下禅定，大小可调。Claude 显示 5H / Weekly / Fable 三个额度，Codex 只显示 Weekly；Grok Build 和 Kimi Code 默认不挤占“屏幕显示”菜单，可在“屏幕显示 → AI 额度设置”中开启后固定显示。同时统计本机时间 00:01 到 23:59 的 token 和模型折算金额。AUTO 空闲时保持最近活跃项，不再无意义轮播。Fast / Priority 模式的每次任务会触发 2.4 秒 Ludicrous 全屏动画。 |
| <img src="docs/images/feature2.jpg" width="360" alt="多市场行情"> | **多市场行情**<br>同一页支持 BTC/ETH、A 股、港股、美股和韩股，以及上证、恒生、SPX、NDX、AAPL、NVDA、KOSPI 等常用标的。收藏列表最多 15 个；右键菜单的“行情设置”统一提供 K线周期、刷新间隔、行情标的和搜索/添加。下一标的会提前加载，完整校验后一次切屏，单个接口失败会自动跳过而不拖住轮换。腾讯/东方财富/Naver 可直连中国大陆网络。 |
...
- **想换桌宠**：右键托盘图标 → 「桌宠外观」依次提供经典宠物、咖色边牧和自定义；选择
  「自定义…」即可从 petdex 挑选并上传动画。
